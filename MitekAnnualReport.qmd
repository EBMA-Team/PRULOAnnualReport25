...
**EBMAReference**: RC_MitekReporting_KE005Nov23
...
Table A: Glossary of terms used in report
...
Data was imported from the PRULO registry for the specified date: **30-Jun-2024** Configuration tables were also loaded, including the product table to describe the products of interest and list identifiers to match to surgical records. The table was modified to include a material column (PEEK or BR) and product identifiers.

```{r Registry snapshot}
## Load in registry snapshot
#| label: load snapshot
#| code-summary: "Input snapshot"
#| warning: false

CurrentDate <- as.character("20240630")


#This function reads three specific sheets from a Registry data snapshot file with the given date and processes them by adding a 'Cohort' column to each data frame with the respective cohort name. The processed data frames are then stored in separate variables: SnapshotRC, SnapshotGH, SnapshotGen, STROBEInput.

SnapshotRC <- read_xlsx(paste0("G:/.shortcut-targets-by-id/0B0o4RQ83QnoDdjdOWWFhcWpnbGc/EBMA/Client Drive/Kevin Eng/GO Shared Folder/Registry snapshots/",CurrentDate,"/Registry data snapshot.xlsx"), sheet = "RotatorCuff", colNames = TRUE, detectDates = TRUE) %>% mutate(
  Cohort = "Rotator Cuff"
)

SnapshotGHI <- read_xlsx(paste0("G:/.shortcut-targets-by-id/0B0o4RQ83QnoDdjdOWWFhcWpnbGc/EBMA/Client Drive/Kevin Eng/GO Shared Folder/Registry snapshots/",CurrentDate,"/Registry data snapshot.xlsx"), sheet = "GlenohumeralInstability", colNames = TRUE, detectDates = TRUE) %>% mutate(
  Cohort = "Glenohumeral Instability"
)

SnapshotGen <- read_xlsx(paste0("G:/.shortcut-targets-by-id/0B0o4RQ83QnoDdjdOWWFhcWpnbGc/EBMA/Client Drive/Kevin Eng/GO Shared Folder/Registry snapshots/",CurrentDate,"/Registry data snapshot.xlsx"), sheet = "ShoulderGeneral", colNames = TRUE, detectDates = TRUE) %>% mutate(
  Cohort = "General"
)

STROBEInput <- read_xlsx(paste0("G:/.shortcut-targets-by-id/0B0o4RQ83QnoDdjdOWWFhcWpnbGc/EBMA/Client Drive/Kevin Eng/GO Shared Folder/Registry snapshots/",CurrentDate,"/Registry data snapshot.xlsx"), sheet = "Strobe_Input", colNames = TRUE, detectDates = TRUE)

```

```{epoxy}

The imported data contains a dataframe of {nrow(SnapshotRC)} cases returned from the Rotator Cuff cohort, a dataframe of {nrow(SnapshotGHI)} cases returned from the Glenohumeral Instability cohort and a dataframe of {nrow(SnapshotGen)} cases from the General cohort. An additional dataframe was retrieved of {nrow(STROBEInput)} cases to generate the STROBE flow chart from the PRULO registry. 

```

<!--# Retrieve data from the live PRULO database. -->

```{r Live tables}
## Load in live database tables
#| label: load registry
#| code-summary: "load registry"
#| warning: false

tic()
# Read ComplicTable from the specified URL and sheet.
# col_names sets column names to TRUE, meaning they will be extracted from the first row.
# col_types specifies the data type for each column. 't' for text, 'c' for character, 'n' for numeric, 
#          'd' for date, and 'D' for datetime. In this case, 'cccDccD' means three columns of text,
#          followed by two columns of date, and then one column of text.
ComplicTable <- googlesheets4::read_sheet("https://docs.google.com/spreadsheets/d/1zyFuf0Wmij13ELTYNmdi_BprF8hU4QSCPsNZ0EYRXzo/edit", 
                           sheet = "Complications",
                           col_names = TRUE, 
                           col_types = "tcccDccD")

# Read ProductTable from the specified URL and sheet. Similar to ComplicTable, it reads a range of columns
#          'F1:J' from the specified sheet 'Mitek'.
# The rest of the arguments are similar to those used in read_sheet for ComplicTable.
ProductTable <- googlesheets4::range_read("https://docs.google.com/spreadsheets/d/1MvT00sc8FzH5SXEi-yzEgW1H1ao3AOoakKqpg7UkxW4/edit", 
                           sheet = "Mitek", 
                           range = "F1:J",
                           col_names = TRUE, 
                           col_types = "ncccc")
...
RefIgnore <- googlesheets4::range_read("https://docs.google.com/spreadsheets/d/1MvT00sc8FzH5SXEi-yzEgW1H1ao3AOoakKqpg7UkxW4/edit", 
                           sheet = "RefIgnore", 
                           range = "A1:A",
                           col_names = TRUE, 
                           col_types = "c")
...
# Similar to previous read functions, it also infers that all entries are of type 'character', except for
#          the second column which is inferred as 'numeric'.
PROMsMaster <- googlesheets4::range_read("https://docs.google.com/spreadsheets/d/1zyFuf0Wmij13ELTYNmdi_BprF8hU4QSCPsNZ0EYRXzo/edit", 
                          sheet = "PROMS Master list", 
                          range = "D1:H",
                          col_names = TRUE, 
                          col_types = "Dcicc") %>% dplyr::select(-`External Reference`)

# Measure time elapsed between tic() and toc()
toc()
...
# 'grepl' is used to check if a string contains a pattern. If the 'Description' column contains "BR",
...
SnapshotComb <- SnapshotRC %>% dplyr::select(-(c(ExternalStudyTag,Email,Phone, ICCount:ICPostopManagement))) %>% 
  # Combine 'SnapshotGH' and 'SnapshotGen' tables with 'SnapshotRC' table by removing the specified columns.
  bind_rows(dplyr::select(SnapshotGHI, -(c(ExternalStudyTag,Email,Phone, ICCount:ICPostopManagement)))) %>% 
  # Continue to combine tables by removing the specified columns. This time, combining 'SnapshotGen'.
  bind_rows(dplyr::select(SnapshotGen, -(c(ExternalStudyTag,Email,Phone, ICCount:ICPostopManagement)))) %>% 
  # Create a new column 'PatientID' based on the TreatmentUID column, using 'str_split_i' function with a pattern of '.'. This means splitting the string at the first occurrence of '.', and capturing the first part (using `1` argument).
  mutate(
        PatientID = str_split_i(TreatmentUID,"\\.",1)
      ) %>% 
  # Move 'PatientID' column before 'TreatmentUID' column.
  relocate(
  ...
  # Move 'CombID' column after 'TreatmentUID' column.
  relocate(
  ...
  DurationTotal = as.numeric(as.duration(interval(ymd(DateTreatment), ymd(CurrentDate))),"days")
  ) %>% filter(
    DurationTotal > 89
  )
...
TargetTerms    <- ProcedureMaster %>% 
  # Select only TreatmentUID and Procedures3 columns
  dplyr::select(
    TreatmentUID,
    Procedures3
  )   %>% 
  # Remove duplicate rows while keeping all other columns (TreatmentUID) using distinct()
  distinct(
    Procedures3, 
    .keep_all = TRUE
  )   %>% 
...
#  data = TargetTerms %>% dplyr::select(TreatmentUID, Procedures3), 
#  sheet = "Procedures", 
...
ProcedureMaster1 <- ProcedureMaster %>% 
  left_join(.,
  ...
  # Group the data by TreatmentUID
...
Figure A: Summary of procedure terms included in registry output
...
ProcedureMaster2 <- left_join(
  ...
  dplyr::select(TreatmentUID, CuffRepair2, AdjunctProcedures2, SoftTissueProcedure, RepairAugmentation, ProceduresOfInterest),
  
  # Summarize ProcedureMaster1a by TreatmentUID and collapse unique ReplaceProcedure values into a single string separated by ";"
...
```{r, eval = FALSE}
SnapshotCombFail <- SnapshotComb2 %>% filter(
  TreatmentStatus == "Failed",
  SurgicalTreatment == "Surgical"
)

googlesheets4::range_write(ss = "https://docs.google.com/spreadsheets/d/1e7431oSrK9tItiSZb4wCWPI7nqGqawH8pNgnqgHPNkU/edit",
...
ComplicTable1 <- rename(ComplicTable, 
                        TreatmentUID = `Treatment UID`) %>%
  filter(
    `Date of Occurrence` < ymd(CurrentDate)
    ) %>%
  mutate(
    TreatmentUID = trimws(TreatmentUID)
  )

#Tidy up TreatmentUID in both tables
# Remove leading and trailing whitespaces

SnapshotComb2a$TreatmentUID <- trimws(SnapshotComb2a$TreatmentUID)
# Check data types
#str(ComplicTable1$TreatmentUID)
#str(SnapshotComb2$TreatmentUID)

#str(SnapshotComb2$DateTreatment)

# Check for NAs in ComplicTable1
na_in_ComplicTable1 <- sum(is.na(ComplicTable1$TreatmentUID))


# Check for NAs in SnapshotMerge2
na_in_SnapshotComb2a <- sum(is.na(SnapshotComb2a$TreatmentUID))

```

```{epoxy, eval = FALSE}

There are {nrow(SnapshotComb2a %>% filter(is.na(TreatmentUID)))} cases with missing TreatmentUID in the dataset and {nrow(ComplicTable1 %>% filter(is.na(TreatmentUID)))} missing in the complications table.
...
```{r}
mismatched_rows <- ComplicTable1[!(ComplicTable1$TreatmentUID %in% SnapshotComb2$TreatmentUID), ]

# Remove non-matching rows from ComplicTable2
ComplicTable2 <- ComplicTable1 %>%
  filter(!(TreatmentUID %in% mismatched_rows$TreatmentUID))
```
...
googlesheets4::range_write(ss = "https://docs.google.com/spreadsheets/d/1EVtJwG2d4UhR4-VGbA0-ZSwdgSgNMbth5kMTsKR_mwI/edit",
                           data = ComplicTable3,
                            sheet = "ComplicTable",
                           range = "A1:AC",
                            col_names = TRUE
...
ComplicInfection <- ComplicTable3 %>% group_by(TreatmentUID) %>% filter(Infection == "Yes") %>% slice_min(DateOccurFix, n = 1) %>% ungroup
ComplicLigament_Tendon <- ComplicTable3 %>% group_by(TreatmentUID) %>% filter(Ligament_Tendon == "Yes") %>% slice_min(DateOccurFix, n = 1) %>% ungroup
ComplicStiffness <- ComplicTable3 %>% group_by(TreatmentUID) %>% filter(Stiffness == "Yes") %>% slice_min(DateOccurFix, n = 1) %>% ungroup
ComplicLoosening <- ComplicTable3 %>% group_by(TreatmentUID) %>% filter(Loosening == "Yes") %>% slice_min(DateOccurFix, n = 1) %>% ungroup
ComplicHardware <- ComplicTable3 %>% group_by(TreatmentUID) %>% filter(Hardware == "Yes" & Loosening == "No") %>% slice_min(DateOccurFix, n = 1) %>% ungroup
ComplicInstability <- ComplicTable3 %>% group_by(TreatmentUID) %>% filter(Instability == "Yes") %>% slice_min(DateOccurFix, n = 1) %>% ungroup
ComplicNeurological <- ComplicTable3 %>% group_by(TreatmentUID) %>% filter(Neurological == "Yes") %>% slice_min(DateOccurFix, n = 1) %>% ungroup()
ComplicFracture <- ComplicTable3 %>% group_by(TreatmentUID) %>% filter(Fracture == "Yes") %>% slice_min(DateOccurFix, n = 1) %>% ungroup()
ComplicThrombosis <- ComplicTable3 %>% group_by(TreatmentUID) %>% filter(Thrombosis == "Yes") %>% slice_min(DateOccurFix, n = 1) %>% ungroup()
ComplicEffusion <- ComplicTable3 %>% group_by(TreatmentUID) %>% filter(Effusion == "Yes") %>% slice_min(DateOccurFix, n = 1) %>% ungroup()
ComplicPain <- ComplicTable3 %>% group_by(TreatmentUID) %>% filter(PainIsol == "Yes") %>% slice_min(DateOccurFix, n = 1) %>% ungroup()
ComplicOther <- ComplicTable3 %>% group_by(TreatmentUID) %>% filter(Other == "Yes") %>% slice_min(DateOccurFix, n = 1) %>% ungroup()
ComplicReop <- ComplicTable3 %>% group_by(TreatmentUID) %>% filter(`Reoperation Procedure` == "Yes") %>% slice_min(DateOccurFix, n = 1) %>% ungroup()
ComplicIntraop <- ComplicTable3 %>% group_by(TreatmentUID) %>% filter(Intraop == "Yes") %>% slice_min(DateOccurFix, n = 1) %>% ungroup()
...
ComplicDiscrep2 <- ComplicTable3 %>%
  group_by(TreatmentUID, DateOccurFix) %>%
  filter(n() > 1)

# Identify the problematic codes that are not linking back to the ProductTable correctly

googlesheets4::range_write(ss = "https://docs.google.com/spreadsheets/d/1EVtJwG2d4UhR4-VGbA0-ZSwdgSgNMbth5kMTsKR_mwI/edit",
                           data = bind_rows(ComplicDiscrep,ComplicDiscrep2) %>% arrange(TreatmentUID),
                           sheet = "ComplicDiscrep",
                           range = "A1:AD",
                           col_names = TRUE
)
...
# Comment out after first use
 ...
 range_write(SnapshotComb2a %>%
               ...
               dplyr::select(TreatmentUID,DateTreatment),
             ss = "https://docs.google.com/spreadsheets/d/1e7431oSrK9tItiSZb4wCWPI7nqGqawH8pNgnqgHPNkU/edit",
...
ProductUsage1 <- SnapshotComb2a %>% dplyr::select(TreatmentUID,ImplantCodes) %>% filter(grepl("10886",ImplantCodes))
...
ProductUsage2 <- separate_longer_delim(
    ...
    cols = ImplantCodes,
    ", ") %>%
  left_join(.,dplyr::select(ProductTable,Reference,Description), 
            join_by(ImplantCodes == Reference)) %>%
  mutate(
    CustomUID = str_remove_all(paste0(Description,"_",TreatmentUID,sep=""),"\\([^\\)]+\\)")
  )
```
...
```{r Product Tracking 1, eval = TRUE}
...
ProductUsage3 <- ProductUsage2b %>% distinct(CustomUID, .keep_all = TRUE) %>% filter(grepl("10886",ImplantCodes)) %>%
  rowwise() %>%
   mutate( #Swap around paste0 of TreatmentUID and Description so that str_count doesnt overlap for customUIDs with other UIDs
    CustomUID = str_remove_all(paste0(Description,"_",TreatmentUID,sep=""),"\\([^\\)]+\\)")
  ) %>%
  rowwise() %>%
  mutate(
...
```{r Combine Product and Procedure}
...
lengths_QDASH <- sapply(QDASH_cols, function(col) length(unique(SnapshotPROM$TreatmentUID)))
lengths_DeltaQ <- sapply(DeltaQ_cols, function(col) length(unique(SnapshotPROM$TreatmentUID)))
lengths_WORC <- sapply(WORC_cols, function(col) length(unique(SnapshotPROM$TreatmentUID)))
lengths_WOSI <- sapply(WOSI_cols, function(col) length(unique(SnapshotPROM$TreatmentUID)))

lengths_Eligible <- sapply(Eligible_cols, function(col) length(unique(SnapshotPROM$TreatmentUID)))

```

```{r PROMs Prep DASH_WOSI}
...
STROBEInput1 <- STROBEInput %>% mutate(
  Cohort = case_when(
    str_detect(RegistryCohortName,"Glenohumeral*") ~ "Glenohumeral Instability",
    str_detect(RegistryCohortName,"General") ~ "General",
    str_detect(RegistryCohortName,"Rotator*") ~ "Rotator Cuff",
    .default = NA_character_
  )
)

CONSORTData <- STROBEInput1  %>% mutate(
  exc1 = case_when(
    grepl("opt-out|decease*",stringr::str_to_lower(RegistryStatus)) ~ RegistryStatus,
    .default = NA_character_
  ),
  induc = case_when(
    grepl("opt-out|decease*",stringr::str_to_lower(RegistryStatus)) ~ NA_character_,
    .default = TreatmentUID
  ),
  exc2 = case_when(
    grepl("archive*|pending*",stringr::str_to_lower(TreatmentStatus)) ~ TreatmentStatus,
    .default = NA_character_
  ),
  exc = case_when(
    is.na(exc1) & is.na(exc2) ~ NA_character_,
    !is.na(exc1) & is.na(exc2) ~ exc1,
    is.na(exc1) & !is.na(exc2) ~ exc2,
    !is.na(exc1) & !is.na(exc2) ~ exc1,
    .default = NA_character_
  ),
  arm = SurgicalTreatment,
  fow1 = case_when(
    grepl("\\bfail*|\\bfurther\\b",stringr::str_to_lower(TreatmentStatus)) ~ TreatmentStatus,
    .default = NA_character_
  ),
  arm3 = Cohort
) %>%
...
# Work out registry level exclusions
# Break into cohorts
# Establish cases with Product of interest
# Work out any exclusions for analysis overall
# Progress through
  # Those eligible for survival and complications analysis
  # Those eligible for PROMs follow up at 12months


SnapshotConsort <- SnapshotComb %>% dplyr::select(
  TreatmentUID,
  DateTreatment,
  EligibleAtx12months
) %>% mutate(
...
#Slice down MasterPROM for 12month eligible

Eligible12m <- SnapshotConsort %>% filter(
  EligibleAtx12months == "Yes"
)

CONSORTData <- STROBEInput1 %>% left_join(
  .,
  SnapshotConsort %>% dplyr::select(
    TreatmentUID,
    DurationTotal
  ),
  by = "TreatmentUID"
) %>% rename(
  trialno = TreatmentUID #List of the subject ID.
) %>% mutate (
  exc1 = case_when( # reason for exclusion after enrolled, should be missing if not exclude
    grepl("opt-out^|decease*",stringr::str_to_lower(RegistryStatus)) ~ RegistryStatus,
    grepl("archive*|pending*",stringr::str_to_lower(TreatmentStatus)) ~ TreatmentStatus,
    .default = NA_character_
  ),
  exc2 = case_when( #exc2: reason for exclusion in the induction phase, should be missing if not excluded.

    SurgicalTreatment == "Non-Surgical" ~ SurgicalTreatment,
    .default = NA_character_
  ),
  exc = case_when( #exc: reason for exclusion after enrolled. This is the combination of exc1 and exc2.
    is.na(exc1) & is.na(exc2) ~ NA_character_,
    !is.na(exc1) & is.na(exc2) ~ exc1,
    is.na(exc1) & !is.na(exc2) ~ exc2,
    !is.na(exc1) & !is.na(exc2) ~ exc1,
  ...
  induc = case_when( # list of subject ID entered the induction phase
    is.na(exc) ~ NA_character_,
    .default = trialno
  ),
    fow1 = case_when( #reason for exclusion in the follow-up, should be missing if not excluded.
    #!(trialno %in% SnapshotComb4$TreatmentUID) ~ "Did not receive Product",
    is.na(exc) & (DurationTotal < 90) ~ "Insufficient mFU", #minimum follow up (3months)
    .default = NA_character_
  ),
  fow2 = case_when( #reason for exclusion in the final analysis, should be missing if not excluded
    !(trialno %in% Eligible12m$TreatmentUID) & (trialno %in% SnapshotConsort$TreatmentUID) ~ "Insufficient fFU", # final follow up (12months)
    .default = NA_character_
  ),
  arm3 = Cohort # treatment arm, three groups.
)
...
ConsortFlow <- consort_plot(
  data = CONSORTData,
  orders = list(
    c(trialno = "Registry population"),
    c(exc1    = "Inactive | Excluded"),
    c(induc   = "Active"),
    c(exc2    = "Excluded"),
    c(arm3     = "Surgical Treatment"),
    c(fow1    = "Excluded"),
    c(trialno = "Eligible"),
    c(fow2    = "Excluded"),
    c(trialno = "Available for Analysis")
    ),
  side_box = c("exc1", "exc2", "fow1", "fow2"),
  allocation = c("arm3"),
  labels = c(
    "1" = "Registry enrolment", 
    "2" = "Active records",
    "3" = "Cohort Allocation", 
    "5" = "Eligible Analysis",
    "6" = "Eligible 12m PROMs"),
  cex = 0.5,
  text_width = 20
  )

knitr::knit_print(ConsortFlow)


```

Figure B: Flow chart of record retrieval and screening for eligibility into report analysis pool
...
Table A: Summary of diagnoses using ICD-10 coding for primary presentation
...
TableData <- CONSORTData %>%
  filter(SurgicalTreatment == "Surgical" & !is.na(ICD10)
         ) %>% group_by(
           Cohort
           ) %>% count(
             ICD10, 
...
knitr::knit_print(TableData)


```

The overall registry enrolment is summarised below, describing how the population is summarised into cohorts.
...
Table B: Summary of PRULO surgical cases by cohort

```{r}

flextable::set_table_properties(TableRepB,
                                layout = "autofit",
                                width = 0.15,
                                align = "center",
                                opts_word = list(
...
SelectRep1 <- SnapshotComb4 %>% 
  ...
  Infection = TreatmentUID %in% ComplicInfection$TreatmentUID,
  Ligament_Tendon = TreatmentUID %in% ComplicLigament_Tendon$TreatmentUID,
  Stiffness = TreatmentUID %in% ComplicStiffness$TreatmentUID,
  Loosening = TreatmentUID %in% ComplicLoosening$TreatmentUID,
  Hardware = TreatmentUID %in% ComplicHardware$TreatmentUID,
  Instability = TreatmentUID %in% ComplicInstability$TreatmentUID,
  Neurological = TreatmentUID %in% ComplicNeurological$TreatmentUID,
  Fracture = TreatmentUID %in% ComplicFracture$TreatmentUID,
  Thrombosis = TreatmentUID %in% ComplicThrombosis$TreatmentUID,
  Effusion = TreatmentUID %in% ComplicEffusion$TreatmentUID,
  Pain = TreatmentUID %in% ComplicPain$TreatmentUID,
  Other = TreatmentUID %in% ComplicOther$TreatmentUID,
  Intraop = TreatmentUID %in% ComplicIntraop$TreatmentUID,
  Reop = TreatmentUID %in% ComplicReop$TreatmentUID
) %>% left_join(
  .,
  ComplicTable3 %>% group_by(TreatmentUID) %>% slice_min(DateOccurFix, n = 1) %>% ungroup %>% dplyr::select(
    TreatmentUID,
    ReopDelay_Occur,
    ReopDelay_Surgery,
    EventDelay
...

{
  if(nrow(SelectRep1) == 1) {
    ...
    procedures_string <- if(length(procedures) > 1) {
      paste(procedures[-length(procedures)], collapse = "] [") %>%
        paste0(., "], and [", procedures[length(procedures)])
    } else {
      procedures
    }
...
Table 1: Summary of PRULO Report 1 (Rotator Cuff) Case - Patient Characteristics
...
flextable::set_table_properties(TableRep1RC1,
                                layout = "autofit",
                                width = 0.15,
                                align = "center",
                                opts_word = list(
...
Table 2: Summary of PRULO Report 1 (Rotator Cuff) Cases Pathology and Surgical Details

```{r}

flextable::set_table_properties(TableRep1RC2,
                                layout = "autofit",
                                width = 0.15,
                                align = "center",
                                opts_word = list(
...
Table 3: Summary of PRULO Report 1 (Rotator Cuff) Procedure Survival
...
flextable::set_table_properties(TableRep1RC3,
                                layout = "autofit",
                                width = 0.15,
                                align = "center",
                                opts_word = list(
...
Table 4: Summary of PRULO Report 1 (Rotator Cuff) Cases - Adverse Events. Complications are postoperative unless otherwise indicated.

```{r}

flextable::set_table_properties(TableRep1RC4,
                                layout = "autofit",
                                #width = 0.35,
                                align = "center",
                                opts_word = list(
...
```{r Reoperations Report Rep1A Proc1}
...
SelectRep1RCReop <- ComplicTable3 %>% filter(
  TreatmentUID %in% SelectRep1RC$TreatmentUID,
  `Reoperation Procedure` == "Yes"
  ) %>% group_by(TreatmentUID) %>% slice_min(
    ReopDelay_Surgery,
    n = 1
    ) %>% ungroup() %>% left_join(
      .,
...
```{epoxy Reoperations Report Rep1B Proc1}
...
```{r PROMs - complete case analysis Rep1 Proc1}
# Plot what we have
...
Table 5: Summary of PRULO Report 1 (Rotator Cuff) Cases - QuickDASH

```{r}

#| label: tbl-PROMs
#| tbl-cap-location: top
#| tbl-cap: "Complete case analysis of QDASH, WORC Normalised and WORC Physical Question 3."
...
flextable::set_table_properties(TableRep1RC5,
                                layout = "autofit",
                                #width = 0.35,
                                align = "center",
                                opts_word = list(
...
Table 6: Summary of PRULO Report 1 (Rotator Cuff) Cases - Western Ontario Rotator Cuff Index
...
tbl_summary(by = "Timepoint",
            ...
            statistic = all_continuous() ~ "{median} ({p25} - {p75})")  %>% as_flex_table()

```

```{r}

flextable::set_table_properties(TableRep1RC6,
                                layout = "autofit",
                                #width = 0.35,
                                align = "center",
                                opts_word = list(
...
Table 7: Summary of PRULO Report 1 (Biceps Tenodesis) Case - Patient Characteristics
...
flextable::set_table_properties(TableRep1BT1,
                                layout = "autofit",
                                width = 0.15,
                                align = "center",
                                opts_word = list(
...
Table 8: Summary of PRULO Report 1 (Biceps Tenodesis) Cases Pathology and Surgical Details

```{r}

flextable::set_table_properties(TableRep1BT2,
                                layout = "autofit",
                                #width = 0.15,
                                align = "center",
                                opts_word = list(
...
Table 9: Summary of PRULO Report 1 (Biceps Tenodesis) procedure survival
...
flextable::set_table_properties(TableRep1BT3,
                                layout = "autofit",
                                #width = 0.15,
                                align = "center",
                                opts_word = list(
...
```{epoxy Failure Report Rep1B Proc2}
...
Table 10: Summary of PRULO Report 1 (Biceps Tenodesis) Cases - Adverse Events
...
flextable::set_table_properties(TableRep1BT4,
                                layout = "autofit",
                                #width = 0.35,
                                align = "center",
                                opts_word = list(
...
```{r Reoperations Report Rep1A Proc2}
...
SelectRep1BTReop <- ComplicTable3 %>% filter(
  TreatmentUID %in% SelectRep1BT$TreatmentUID,
  `Reoperation Procedure` == "Yes"
  ) %>% group_by(TreatmentUID) %>% slice_min(
    ReopDelay_Surgery,
    n = 1
...
```{epoxy Reoperations Report Rep1B Proc2}
...
```{r PROMs - complete case analysis Rep1 Proc2}
# Plot what we have
...
Figure 2: Complete case analysis of QDASH(Top), WORC Normalised (Middle), and WORC Physical Question 3 (Bottom).

Table 11: Summary of PRULO Report 1 (Biceps Tenodesis) Cases - QuickDASH
...
flextable::set_table_properties(TableRep1BT5,
                                layout = "autofit",
                                #width = 0.35,
                                align = "center",
                                opts_word = list(
...
Table 12: Summary of PRULO Report 1 (Biceps Tenodesis) Cases - Western Ontario Rotator Cuff Index,
...
flextable::set_table_properties(TableRep1BT6,
                                layout = "autofit",
                                #width = 0.35,
                                align = "center",
                                opts_word = list(
...
Table 1: Summary of PRULO Report 2 (Rotator Cuff) Case - Patient Characteristics
...
flextable::set_table_properties(TableRep2RC1,
                                layout = "autofit",
                                width = 0.15,
                                align = "center",
                                opts_word = list(
...
Table 2: Summary of PRULO Report 2 (Rotator Cuff) Cases Pathology and Surgical Details

```{r}

flextable::set_table_properties(TableRep2RC2,
                                layout = "autofit",
                                width = 0.15,
                                align = "center",
                                opts_word = list(
...
Table 3: Summary of PRULO Report 2 (Rotator Cuff) Procedure Survival
...
flextable::set_table_properties(TableRep2RC3,
                                layout = "autofit",
                                width = 0.15,
                                align = "center",
                                opts_word = list(
...
```{epoxy Failure Report Rep2B Proc1}

{ifelse(nrow(FailuresRep2RC) == 0,
        epoxy::epoxy("There are no failures or revisions to report."),
       googlesheets4::range_write(
         ss = "https://docs.google.com/spreadsheets/d/1EVtJwG2d4UhR4-VGbA0-ZSwdgSgNMbth5kMTsKR_mwI/edit",
         data = FailuresRep2RC %>% arrange(TreatmentUID),
         sheet = "Failures2",
         range = "A1:P",
         col_names = TRUE
         )
...
Table 4: Summary of PRULO Report 2 (Rotator Cuff) Cases - Adverse Events
...
flextable::set_table_properties(TableRep2RC4,
                                layout = "autofit",
                                #width = 0.35,
                                align = "center",
                                opts_word = list(
...
```{r Reoperations Report Rep2A Proc1}
...
SelectRep2RCReop <- ComplicTable3 %>% filter(
  TreatmentUID %in% SelectRep2RC$TreatmentUID,
  `Reoperation Procedure` == "Yes"
  ) %>% group_by(TreatmentUID) %>% slice_min(
    ReopDelay_Surgery,
    n = 1
...
```{epoxy Reoperations Report Rep2B Proc1}
...
```{r PROMs - complete case analysis Rep2 Proc1}
# Plot what we have
...
Table 5: Summary of PRULO Report 2 (Rotator Cuff) Cases - QuickDASH
...
flextable::set_table_properties(TableRep2RC5,
                                layout = "autofit",
                                #width = 0.35,
                                align = "center",
                                opts_word = list(
...
Table 6: Summary of PRULO Report 2 (Rotator Cuff) Cases - Western Ontario Rotator Cuff Index
...
flextable::set_table_properties(TableRep2RC6,
                                layout = "autofit",
                                #width = 0.35,
                                align = "center",
                                opts_word = list(
...
Table 7: Summary of PRULO Report 2 (Biceps Tenodesis) Case - Patient Characteristics

```{r}

flextable::set_table_properties(TableRep2BT1,
                                layout = "autofit",
                                width = 0.15,
                                align = "center",
                                opts_word = list(
...
Table 8: Summary of PRULO Report 2 (Biceps Tenodesis) Cases Pathology and Surgical Details

```{r}

flextable::set_table_properties(TableRep2BT2,
                                layout = "autofit",
                                #width = 0.15,
                                align = "center",
                                opts_word = list(
...
Table 9: Summary of PRULO Report 2 (Biceps Tenodesis) procedure survival
...
flextable::set_table_properties(TableRep2BT3,
                                layout = "autofit",
                                #width = 0.15,
                                align = "center",
                                opts_word = list(
...
```{epoxy Failure Report Rep2B Proc2}
...
Table 10: Summary of PRULO Report 2 (Biceps Tenodesis) Cases - Adverse Events
...
flextable::set_table_properties(TableRep2BT4,
                                layout = "autofit",
                                #width = 0.35,
                                align = "center",
                                opts_word = list(
...
```{r Reoperations Report Rep2A Proc2}
...
SelectRep2BTReop <- ComplicTable3 %>% filter(
  TreatmentUID %in% SelectRep2BT$TreatmentUID,
  `Reoperation Procedure` == "Yes"
  ) %>% group_by(TreatmentUID) %>% slice_min(
    ReopDelay_Surgery,
    n = 1
...
```{epoxy Reoperations Report Rep2B Proc2}
...
```{r PROMs - complete case analysis Rep2 Proc2}
# Plot what we have
...
Table 11: Summary of PRULO Report 2 (Biceps Tenodesis) Cases - QuickDASH
...
flextable::set_table_properties(TableRep2BT5,
                                layout = "autofit",
                                #width = 0.35,
                                align = "center",
                                opts_word = list(
...
Table 12: Summary of PRULO Report 2 (Biceps Tenodesis) Cases - Western Ontario Rotator Cuff Index,
...
flextable::set_table_properties(TableRep2BT6,
                                layout = "autofit",
                                #width = 0.35,
                                align = "center",
                                opts_word = list(
...
Table 1: Summary of PRULO Report 3 (Rotator Cuff) Case - Patient Characteristics
...
flextable::set_table_properties(TableRep3RC1,
                                layout = "autofit",
                                width = 0.15,
                                align = "center",
                                opts_word = list(
...
Table 2: Summary of PRULO Report 3 (Rotator Cuff) Cases Pathology and Surgical Details

```{r}

flextable::set_table_properties(TableRep3RC2,
                                layout = "autofit",
                                width = 0.15,
                                align = "center",
                                opts_word = list(
...
Table 3: Summary of PRULO Report 3 (Rotator Cuff) Procedure Survival
...
flextable::set_table_properties(TableRep3RC3,
                                layout = "autofit",
                                width = 0.15,
                                align = "center",
                                opts_word = list(
...
```{epoxy Failure Report Rep3B Proc1}

{ifelse(nrow(FailuresRep3RC) == 0,
        epoxy::epoxy("There are no failures or revisions to report."),
       googlesheets4::range_write(
         ss = "https://docs.google.com/spreadsheets/d/1EVtJwG2d4UhR4-VGbA0-ZSwdgSgNMbth5kMTsKR_mwI/edit",
         data = FailuresRep3RC %>% arrange(TreatmentUID),
         sheet = "Failures3",
         range = "A1:P",
         col_names = TRUE
         )
...
Table 4: Summary of PRULO Report 3 (Rotator Cuff) Cases - Adverse Events
...
flextable::set_table_properties(TableRep3RC4,
                                layout = "autofit",
                                #width = 0.35,
                                align = "center",
                                opts_word = list(
...
```{r Reoperations Report Rep3A Proc1}
...
SelectRep3RCReop <- ComplicTable3 %>% filter(
  TreatmentUID %in% SelectRep3RC$TreatmentUID,
  `Reoperation Procedure` == "Yes"
  ) %>% group_by(TreatmentUID) %>% slice_min(
    ReopDelay_Surgery,
    n = 1
...
```{epoxy Reoperations Report Rep3B Proc1}



{ifelse(nrow(SelectRep3RCReop) == 0,
        epoxy::epoxy("There are no reoperations to report."),
       googlesheets4::range_write(
         ss = "https://docs.google.com/spreadsheets/d/1EVtJwG2d4UhR4-VGbA0-ZSwdgSgNMbth5kMTsKR_mwI/edit",
...
```{r PROMs - complete case analysis Rep3 Proc1}
# Plot what we have
...
flextable::set_table_properties(TableRep3RC5,
                                layout = "autofit",
                                #width = 0.35,
                                align = "center",
                                opts_word = list(
...
Table 6: Summary of PRULO Report 3 (Rotator Cuff) Cases - Western Ontario Rotator Cuff Index
...
flextable::set_table_properties(TableRep3RC6,
                                layout = "autofit",
                                #width = 0.35,
                                align = "center",
                                opts_word = list(
...
Table 7: Summary of PRULO Report 3 (Biceps Tenodesis) Case - Patient Characteristics

```{r}

flextable::set_table_properties(TableRep3BT1,
                                layout = "autofit",
                                width = 0.15,
                                align = "center",
                                opts_word = list(
...
Table 8: Summary of PRULO Report 3 (Biceps Tenodesis) Cases Pathology and Surgical Details

```{r}

flextable::set_table_properties(TableRep3BT2,
                                layout = "autofit",
                                #width = 0.15,
                                align = "center",
                                opts_word = list(
...
Table 9: Summary of PRULO Report 3 (Biceps Tenodesis) procedure survival
...
flextable::set_table_properties(TableRep3BT3,
                                layout = "autofit",
                                #width = 0.15,
                                align = "center",
                                opts_word = list(
...
```{epoxy Failure Report Rep3B Proc2}


 {ifelse(nrow(FailuresRep3BT) == 0,
        epoxy::epoxy("There are no failures or revisions to report."),
        with_gs4_quiet(sheet_append(
          ss = "https://docs.google.com/spreadsheets/d/1EVtJwG2d4UhR4-VGbA0-ZSwdgSgNMbth5kMTsKR_mwI/edit",
          data = FailuresRep3BT,
...
Table 10: Summary of PRULO Report 3 (Biceps Tenodesis) Cases - Adverse Events
...
flextable::set_table_properties(TableRep3BT4,
                                layout = "autofit",
                                #width = 0.35,
                                align = "center",
                                opts_word = list(
...
```{r Reoperations Report Rep3A Proc2}
...
SelectRep3BTReop <- ComplicTable3 %>% filter(
  TreatmentUID %in% SelectRep3BT$TreatmentUID,
  `Reoperation Procedure` == "Yes"
  ) %>% group_by(TreatmentUID) %>% slice_min(
    ReopDelay_Surgery,
    n = 1
...
```{epoxy Reoperations Report Rep3B Proc2}

 {ifelse(nrow(SelectRep3BTReop) == 0,
        epoxy::epoxy("There are no reoperations to report."),
        with_gs4_quiet(sheet_append(
          ss = "https://docs.google.com/spreadsheets/d/1EVtJwG2d4UhR4-VGbA0-ZSwdgSgNMbth5kMTsKR_mwI/edit",
...
```{r PROMs - complete case analysis Rep3 Proc2}
# Plot what we have
...
Table 11: Summary of PRULO Report 3 (Biceps Tenodesis) Cases - QuickDASH
...
flextable::set_table_properties(TableRep3BT5,
                                layout = "autofit",
                                #width = 0.35,
                                align = "center",
                                opts_word = list(
...
Table 12: Summary of PRULO Report 3 (Biceps Tenodesis) Cases - Western Ontario Rotator Cuff Index
...
flextable::set_table_properties(TableRep3BT6,
                                layout = "autofit",
                                #width = 0.35,
                                align = "center",
                                opts_word = list(
...
Table 1: Summary of PRULO Report 4 (Rotator Cuff) Case - Patient Characteristics
...
flextable::set_table_properties(TableRep4RC1,
                                layout = "autofit",
                                width = 0.15,
                                align = "center",
                                opts_word = list(
...
Table 2: Summary of PRULO Report 4 (Rotator Cuff) Cases Pathology and Surgical Details

```{r}

flextable::set_table_properties(TableRep4RC2,
                                layout = "autofit",
                                width = 0.15,
                                align = "center",
                                opts_word = list(
...
Table 3: Summary of PRULO Report 4 (Rotator Cuff) Procedure Survival
...
flextable::set_table_properties(TableRep4RC3,
                                layout = "autofit",
                                width = 0.15,
                                align = "center",
                                opts_word = list(
...
```{epoxy Failure Report Rep4B Proc1}

{ifelse(nrow(FailuresRep4RC) == 0,
        epoxy::epoxy("There are no failures or revisions to report."),
       googlesheets4::range_write(
         ss = "https://docs.google.com/spreadsheets/d/1EVtJwG2d4UhR4-VGbA0-ZSwdgSgNMbth5kMTsKR_mwI/edit",
         data = FailuresRep4RC %>% arrange(TreatmentUID),
         sheet = "Failure4",
         range = "A1:P",
         col_names = TRUE
         )
...
Table 4: Summary of PRULO Report 4 (Rotator Cuff) Cases - Adverse Events
...
flextable::set_table_properties(TableRep4RC4,
                                layout = "autofit",
                                #width = 0.35,
                                align = "center",
                                opts_word = list(
...
```{r Reoperations Report Rep4A Proc1}
...
SelectRep4RCReop <- ComplicTable3 %>% filter(
  TreatmentUID %in% SelectRep4RC$TreatmentUID,
  `Reoperation Procedure` == "Yes"
  ) %>% group_by(TreatmentUID) %>% slice_min(
    ReopDelay_Surgery,
    n = 1
...
```{epoxy Reoperations Report Rep4B Proc1}



{ifelse(nrow(SelectRep4RCReop) == 0,
        epoxy::epoxy("There are no reoperations to report."),
       googlesheets4::range_write(
         ss = "https://docs.google.com/spreadsheets/d/1EVtJwG2d4UhR4-VGbA0-ZSwdgSgNMbth5kMTsKR_mwI/edit",
...
```{r PROMs - complete case analysis Rep4 Proc1}
# Plot what we have
...
Table 5: Summary of PRULO Report 4 (Rotator Cuff) Cases - QuickDASH
...
flextable::set_table_properties(TableRep4RC5,
                                layout = "autofit",
                                #width = 0.35,
                                align = "center",
                                opts_word = list(
...
Table 6: Summary of PRULO Report 4 (Rotator Cuff) Cases - Western Ontario Rotator Cuff Index
...
flextable::set_table_properties(TableRep4RC6,
                                layout = "autofit",
                                #width = 0.35,
                                align = "center",
                                opts_word = list(
...
Table 7: Summary of PRULO Report 4 (Biceps Tenodesis) Case - Patient Characteristics

```{r}

flextable::set_table_properties(TableRep4BT1,
                                layout = "autofit",
                                width = 0.15,
                                align = "center",
                                opts_word = list(
...
flextable::set_table_properties(TableRep4BT2,
                                layout = "autofit",
                                #width = 0.15,
                                align = "center",
                                opts_word = list(
...
Table 9: Summary of PRULO Report 4 (Biceps Tenodesis) procedure survival
...
flextable::set_table_properties(TableRep4BT3,
                                layout = "autofit",
                                #width = 0.15,
                                align = "center",
                                opts_word = list(
...
```{epoxy Failure Report Rep4B Proc2}



 {ifelse(nrow(FailuresRep4BT) == 0,
        epoxy::epoxy("There are no failures or revisions to report."),
        with_gs4_quiet(sheet_append(
          ss = "https://docs.google.com/spreadsheets/d/1EVtJwG2d4UhR4-VGbA0-ZSwdgSgNMbth5kMTsKR_mwI/edit",
          data = FailuresRep4BT,
...
Table 10: Summary of PRULO Report 4 (Biceps Tenodesis) Cases - Adverse Events
...
flextable::set_table_properties(TableRep4BT4,
                                layout = "autofit",
                                #width = 0.35,
                                align = "center",
                                opts_word = list(
...
```{r Reoperations Report Rep4A Proc2}
...
SelectRep4BTReop <- ComplicTable3 %>% filter(
  TreatmentUID %in% SelectRep4BT$TreatmentUID,
  `Reoperation Procedure` == "Yes"
  ) %>% group_by(TreatmentUID) %>% slice_min(
    ReopDelay_Surgery,
    n = 1
...
```{epoxy Reoperations Report Rep4B Proc2}

 {ifelse(nrow(SelectRep4BTReop) == 0,
        epoxy::epoxy("There are no reoperations to report."),
        with_gs4_quiet(sheet_append(
          ss = "https://docs.google.com/spreadsheets/d/1EVtJwG2d4UhR4-VGbA0-ZSwdgSgNMbth5kMTsKR_mwI/edit",
...
```{r PROMs - complete case analysis Rep4 Proc2}
# Plot what we have
...
Table 11: Summary of PRULO Report 4 (Biceps Tenodesis) Cases - QuickDASH
...
flextable::set_table_properties(TableRep4BT5,
                                layout = "autofit",
                                #width = 0.35,
                                align = "center",
                                opts_word = list(
...
Table 12: Summary of PRULO Report 4 (Biceps Tenodesis) Cases - Western Ontario Rotator Cuff Index
...
flextable::set_table_properties(TableRep4BT6,
                                layout = "autofit",
                                #width = 0.35,
                                align = "center",
                                opts_word = list(
...
Table 1: Summary of PRULO Report 5 (Rotator Cuff) Case - Patient Characteristics
...
flextable::set_table_properties(TableRep5RC1,
                                layout = "autofit",
                                width = 0.15,
                                align = "center",
                                opts_word = list(
...
Table 2: Summary of PRULO Report 5 (Rotator Cuff) Cases Pathology and Surgical Details

```{r}

flextable::set_table_properties(TableRep5RC2,
                                layout = "autofit",
                                width = 0.15,
                                align = "center",
                                opts_word = list(
...
Table 3: Summary of PRULO Report 5 (Rotator Cuff) Procedure Survival
...
flextable::set_table_properties(TableRep5RC3,
                                layout = "autofit",
                                width = 0.15,
                                align = "center",
                                opts_word = list(
...
```{epoxy Failure Report Rep5B Proc1}

{ifelse(nrow(FailuresRep5RC) == 0,
        epoxy::epoxy("There are no failures or revisions to report."),
       googlesheets4::range_write(
         ss = "https://docs.google.com/spreadsheets/d/1EVtJwG2d4UhR4-VGbA0-ZSwdgSgNMbth5kMTsKR_mwI/edit",
         data = FailuresRep5RC %>% arrange(TreatmentUID),
         sheet = "Failures5",
         range = "A1:P",
         col_names = TRUE
         )
...
Table 4: Summary of PRULO Report 5 (Rotator Cuff) Cases - Adverse Events
...
flextable::set_table_properties(TableRep5RC4,
                                layout = "autofit",
                                #width = 0.35,
                                align = "center",
                                opts_word = list(
...
```{r Reoperations Report Rep5A Proc1}
...
SelectRep5RCReop <- ComplicTable3 %>% filter(
  TreatmentUID %in% SelectRep5RC$TreatmentUID,
  `Reoperation Procedure` == "Yes"
  ) %>% group_by(TreatmentUID) %>% slice_min(
    ReopDelay_Surgery,
    n = 1
...
```{epoxy Reoperations Report Rep5B Proc1}



ifelse(nrow(SelectRep5RCReop) == 0,
        epoxy::epoxy("There are no reoperations to report."),
       googlesheets4::range_write(
         ss = "https://docs.google.com/spreadsheets/d/1EVtJwG2d4UhR4-VGbA0-ZSwdgSgNMbth5kMTsKR_mwI/edit",
...
```{r PROMs - complete case analysis Rep5 Proc1}
# Plot what we have
...
Table 5: Summary of PRULO Report 5 (Rotator Cuff) Cases - QuickDASH
...
flextable::set_table_properties(TableRep5RC5,
                                layout = "autofit",
                                #width = 0.35,
                                align = "center",
                                opts_word = list(
...
Table 6: Summary of PRULO Report 5 (Rotator Cuff) Cases - Western Ontario Rotator Cuff Index
...
flextable::set_table_properties(TableRep5RC6,
                                layout = "autofit",
                                #width = 0.35,
                                align = "center",
                                opts_word = list(
...
Table 7: Summary of PRULO Report 5 (Biceps Tenodesis) Case - Patient Characteristics

```{r}

flextable::set_table_properties(TableRep5BT1,
                                layout = "autofit",
                                width = 0.15,
                                align = "center",
                                opts_word = list(
...
flextable::set_table_properties(TableRep5BT2,
                                layout = "autofit",
                                #width = 0.15,
                                align = "center",
                                opts_word = list(
...
Table 9: Summary of PRULO Report 5 (Biceps Tenodesis) procedure survival
...
flextable::set_table_properties(TableRep5BT3,
                                layout = "autofit",
                                #width = 0.15,
                                align = "center",
                                opts_word = list(
...
```{epoxy Failure Report Rep5B Proc2}


 {ifelse(nrow(FailuresRep5BT) == 0,
        epoxy::epoxy("There are no failures or revisions to report."),
        with_gs4_quiet(sheet_append(
          ss = "https://docs.google.com/spreadsheets/d/1EVtJwG2d4UhR4-VGbA0-ZSwdgSgNMbth5kMTsKR_mwI/edit",
          data = FailuresRep5BT %>% dplyr::select(-QDASH_TotalScore_3months),
          sheet = "Failures5"
        )
        )
 )}
...
Table 10: Summary of PRULO Report 5 (Biceps Tenodesis) Cases - Adverse Events
...
flextable::set_table_properties(TableRep5BT4,
                                layout = "autofit",
                                #width = 0.35,
                                align = "center",
                                opts_word = list(
...
```{r Reoperations Report Rep5A Proc2}
...
SelectRep5BTReop <- ComplicTable3 %>% filter(
  TreatmentUID %in% SelectRep5BT$TreatmentUID,
  `Reoperation Procedure` == "Yes"
  ) %>% group_by(TreatmentUID) %>% slice_min(
    ReopDelay_Surgery,
    n = 1
...
```{epoxy Reoperations Report Rep5B Proc2}

 {ifelse(nrow(SelectRep5BTReop) == 0,
        epoxy::epoxy("There are no reoperations to report."),
        with_gs4_quiet(sheet_append(
          ss = "https://docs.google.com/spreadsheets/d/1EVtJwG2d4UhR4-VGbA0-ZSwdgSgNMbth5kMTsKR_mwI/edit",
...
```{r PROMs - complete case analysis Rep5 Proc2}
# Plot what we have
...
Table 11: Summary of PRULO Report 5 (Biceps Tenodesis) Cases - QuickDASH
...
flextable::set_table_properties(TableRep5BT5,
                                layout = "autofit",
                                #width = 0.35,
                                align = "center",
                                opts_word = list(
...
Table 12: Summary of PRULO Report 5 (Biceps Tenodesis) Cases - Western Ontario Rotator Cuff Index
...
flextable::set_table_properties(TableRep5BT6,
                                layout = "autofit",
                                #width = 0.35,
                                align = "center",
                                opts_word = list(
...
Table 1: Summary of PRULO Report 6 (All) - Patient Characteristics
...
flextable::set_table_properties(TableRep61,
                                layout = "autofit",
                                width = 0.15,
                                align = "center",
                                opts_word = list(
...
Table 2: Summary of PRULO Report 6 (All) Cases Pathology and Surgical Details

```{r}

flextable::set_table_properties(TableRep62,
                                layout = "autofit",
                                width = 0.15,
                                align = "center",
                                opts_word = list(
...
Table 3: Summary of PRULO Report 6 (Rotator Cuff) Procedure Survival
...
flextable::set_table_properties(TableRep63,
                                layout = "autofit",
                                width = 0.15,
                                align = "center",
                                opts_word = list(
...
```{epoxy Failure Report Rep6B Proc1}

{ifelse(nrow(FailuresRep6) == 0,
        epoxy::epoxy("There are no failures or revisions to report."),
       googlesheets4::range_write(
         ss = "https://docs.google.com/spreadsheets/d/1EVtJwG2d4UhR4-VGbA0-ZSwdgSgNMbth5kMTsKR_mwI/edit",
         data = FailuresRep6 %>% arrange(TreatmentUID),
         sheet = "Failures6",
         range = "A1:P",
         col_names = TRUE
         )
...
Table 4: Summary of PRULO Report 6 (All) Cases - Adverse Events
...
TableRep64 <- SelectRep6 %>%
  ...
  tbl_summary(
    ...
    digits = list(
      ...
      columns = label,
      rows = label == "Intraoperative Occurrence",
      footnote = "Anchor loosening, immediate replacement"
    ) %>% modify_table_styling (
      x = .,
      columns = label,
      rows = label == "Subsequent Treatment",
...
flextable::set_table_properties(TableRep64,
                                layout = "autofit",
                                #width = 0.35,
                                align = "center",
                                opts_word = list(
...
```{r Reoperations Report Rep6A Proc1}
...
SelectRep6Reop <- ComplicTable3 %>% filter(
  TreatmentUID %in% SelectRep6$TreatmentUID,
  `Reoperation Procedure` == "Yes"
  ) %>% group_by(TreatmentUID) %>% slice_min(
    ReopDelay_Surgery,
    n = 1
...
```{epoxy Reoperations Report Rep6B Proc1}



ifelse(nrow(SelectRep6Reop) == 0,
        epoxy::epoxy("There are no reoperations to report."),
       googlesheets4::range_write(
         ss = "https://docs.google.com/spreadsheets/d/1EVtJwG2d4UhR4-VGbA0-ZSwdgSgNMbth5kMTsKR_mwI/edit",
         data = SelectRep6Reop,
...
```{r PROMs - complete case analysis Rep6 Proc1}
# Plot what we have

#| label: fig-PROMs
#| fig-cap: "Complete case analysis of QDASH(Top), WOSI Normalised (Bottom)."
#| fig-cap-location: bottom
...
Figure 1: Complete case analysis of QDASH(Top), WOSI Index (Bottom).

Table 5: Summary of PRULO Report 6 (All) Cases - QuickDASH

```{r}

#| label: tbl-PROMs
#| tbl-cap-location: top
#| tbl-cap: "Complete case analysis of QDASH, WOSI Normalised."
...
flextable::set_table_properties(TableRep65,
                                layout = "autofit",
                                #width = 0.35,
                                align = "center",
                                opts_word = list(
...
Table 6: Summary of PRULO Report 6 (Bankart Repair) - Patient Characteristics
...
flextable::set_table_properties(TableRep6BR1,
                                layout = "autofit",
                                width = 0.15,
                                align = "center",
                                opts_word = list(
...
Table 7: Summary of PRULO Report 6 (Bankart Repair) Cases Pathology and Surgical Details

```{r}

flextable::set_table_properties(TableRep6BR2,
                                layout = "autofit",
                                width = 0.15,
                                align = "center",
                                opts_word = list(
...
Table 8: Summary of PRULO Report 6 (Bankart Repair) Procedure Survival
...
flextable::set_table_properties(TableRep6BR3,
                                layout = "autofit",
                                width = 0.15,
                                align = "center",
                                opts_word = list(
...
```{r Failure Report Rep6BRA Proc2}
...
```{epoxy Failure Report Rep6B Proc2}


 {ifelse(nrow(FailuresRep6BR) == 0,
        epoxy::epoxy("There are no failures or revisions to report."),
        with_gs4_quiet(sheet_append(
          ss = "https://docs.google.com/spreadsheets/d/1EVtJwG2d4UhR4-VGbA0-ZSwdgSgNMbth5kMTsKR_mwI/edit",
          data = FailuresRep6BR,
...
Table 9: Summary of PRULO Report 6 (Bankart Repair) Cases - Adverse Events
...
flextable::set_table_properties(TableRep6BR4,
                                layout = "autofit",
                                #width = 0.35,
                                align = "center",
                                opts_word = list(
...
```{r Reoperations Report Rep6BRA Proc2}
...
SelectRep6BRReop <- ComplicTable3 %>% filter(
  TreatmentUID %in% SelectRep6BR$TreatmentUID,
  `Reoperation Procedure` == "Yes"
  ) %>% group_by(TreatmentUID) %>% slice_min(
    ReopDelay_Surgery,
    n = 1
...
```{epoxy Reoperations Report Rep6BRB Proc2}



ifelse(nrow(SelectRep6BRReop) == 0,
        epoxy::epoxy("There are no reoperations to report."),
       googlesheets4::range_write(
         ss = "https://docs.google.com/spreadsheets/d/1EVtJwG2d4UhR4-VGbA0-ZSwdgSgNMbth5kMTsKR_mwI/edit",
         data = SelectRep6BRReop,
         sheet = "Reoperations6",
         range = "A1:P",
         col_names = TRUE
         )
...
Complete case analysis of QDASH, WOSI Index Normalised, and WOSI Physical Question 3 (How much weakness do you experience in your shoulder?) is summarised below.

```{r PROMs - complete case analysis Rep6BR Proc1}
# Plot what we have
...
Table 10: Summary of PRULO Report 6 (Bankart Repair) Cases - QuickDASH and WOSI Normalised.
...
flextable::set_table_properties(TableRep6BR5,
                                layout = "autofit",
                                #width = 0.35,
                                align = "center",
                                opts_word = list(
...
Table 1: Summary of PRULO Report 7 (All) - Patient Characteristics
...
flextable::set_table_properties(TableRep71,
                                layout = "autofit",
                                width = 0.15,
                                align = "center",
                                opts_word = list(
...
Table 2: Summary of PRULO Report 7 (All) Cases Pathology and Surgical Details

```{r}

flextable::set_table_properties(TableRep72,
                                layout = "autofit",
                                width = 0.15,
                                align = "center",
                                opts_word = list(
...
Table 3: Summary of PRULO Report 7 (Rotator Cuff) Procedure Survival
...
flextable::set_table_properties(TableRep73,
                                layout = "autofit",
                                width = 0.15,
                                align = "center",
                                opts_word = list(
...
```{epoxy Failure Report Rep7B Proc1}

{ifelse(nrow(FailuresRep7) == 0,
        epoxy::epoxy("There are no failures to report."),
       googlesheets4::range_write(
         ss = "https://docs.google.com/spreadsheets/d/1EVtJwG2d4UhR4-VGbA0-ZSwdgSgNMbth5kMTsKR_mwI/edit",
         data = FailuresRep7 %>% arrange(TreatmentUID),
         sheet = "Failures7",
         range = "A1:P",
         col_names = TRUE
         )
...
Table 4: Summary of PRULO Report 7 (All) Cases - Adverse Events
...
flextable::set_table_properties(TableRep74,
                                layout = "autofit",
                                #width = 0.35,
                                align = "center",
                                opts_word = list(
...
```{r Reoperations Report Rep7A Proc1}
...
SelectRep7Reop <- ComplicTable3 %>% filter(
  TreatmentUID %in% SelectRep7$TreatmentUID,
  `Reoperation Procedure` == "Yes"
  ) %>% group_by(TreatmentUID) %>% slice_min(
    ReopDelay_Surgery,
    n = 1
...
```{epoxy Reoperations Report Rep7B Proc1}



ifelse(nrow(SelectRep7Reop) == 0,
        epoxy::epoxy("There are no reoperations to report."),
       googlesheets4::range_write(
         ss = "https://docs.google.com/spreadsheets/d/1EVtJwG2d4UhR4-VGbA0-ZSwdgSgNMbth5kMTsKR_mwI/edit",
         data = SelectRep7Reop,
...
```{r PROMs - complete case analysis Rep7 Proc1}
# Plot what we have
...
Table 5: Summary of PRULO Report 7 (All) Cases - QuickDASH
...
flextable::set_table_properties(TableRep75,
                                layout = "autofit",
                                #width = 0.35,
                                align = "center",
                                opts_word = list(
...
Table 6: Summary of PRULO Report 7 (Bankart Repair) - Patient Characteristics
...
flextable::set_table_properties(TableRep7BR1,
                                layout = "autofit",
                                width = 0.15,
                                align = "center",
                                opts_word = list(
...
Table 7: Summary of PRULO Report 7 (Bankart Repair) Cases Pathology and Surgical Details

```{r}

flextable::set_table_properties(TableRep7BR2,
                                layout = "autofit",
                                width = 0.15,
                                align = "center",
                                opts_word = list(
...
Table 8: Summary of PRULO Report 7 (Bankart Repair) Procedure Survival
...
flextable::set_table_properties(TableRep7BR3,
                                layout = "autofit",
                                width = 0.15,
                                align = "center",
                                opts_word = list(
...
```{r Failure Report Rep7BRA Proc2}
...
```{epoxy Failure Report Rep7B Proc2}


 {ifelse(nrow(FailuresRep7BR) == 0,
        epoxy::epoxy("There are no failures or revisions to report."),
        with_gs4_quiet(sheet_append(
          ss = "https://docs.google.com/spreadsheets/d/1EVtJwG2d4UhR4-VGbA0-ZSwdgSgNMbth5kMTsKR_mwI/edit",
          data = FailuresRep7BR,
...
Table 9: Summary of PRULO Report 7 (Bankart Repair) Cases - Adverse Events
...
flextable::set_table_properties(TableRep7BR4,
                                layout = "autofit",
                                #width = 0.35,
                                align = "center",
                                opts_word = list(
...
```{r Reoperations Report Rep7BRA Proc2}
...
SelectRep7BRReop <- ComplicTable3 %>% filter(
  TreatmentUID %in% SelectRep7BR$TreatmentUID,
  `Reoperation Procedure` == "Yes"
  ) %>% group_by(TreatmentUID) %>% slice_min(
    ReopDelay_Surgery,
    n = 1
...
```{epoxy Reoperations Report Rep7BRB Proc2}



ifelse(nrow(SelectRep7BRReop) == 0,
        epoxy::epoxy("There are no reoperations to report."),
       googlesheets4::range_write(
         ss = "https://docs.google.com/spreadsheets/d/1EVtJwG2d4UhR4-VGbA0-ZSwdgSgNMbth5kMTsKR_mwI/edit",
         data = SelectRep7BRReop,
         sheet = "Reoperations7",
         range = "A1:P",
         col_names = TRUE
         )
...
```{r PROMs - complete case analysis Rep7BR Proc2}
# Plot what we have
...
Table 10: Summary of PRULO Report 7 (Bankart Repair) Cases - QuickDASH and WOSI Normalised.
...
flextable::set_table_properties(TableRep7BR5,
                                layout = "autofit",
                                #width = 0.35,
                                align = "center",
                                opts_word = list(
...
{ifelse(nrow(SelectRep9BR) <= 1,
        ...
        epoxy::epoxy("There are {nrow(SelectRep9BR)} cases involving the anchor of interest and a Bankart repair. Surgeries were performed between {format(min(SelectRep9BR$DateTreatment),'%Y-%b-%d')} to {format(max(SelectRep8BR$DateTreatment),'%Y-%b-%d')}."
        )))} 
...
Table 1: Summary of PRULO Report 10 (All) - Patient Characteristics
...
flextable::set_table_properties(TableRep101,
                                layout = "autofit",
                                width = 0.15,
                                align = "center",
                                opts_word = list(
...
Table 2: Summary of PRULO Report 10 (All) Cases Pathology and Surgical Details

```{r}

flextable::set_table_properties(TableRep102,
                                layout = "autofit",
                                width = 0.15,
                                align = "center",
                                opts_word = list(
...
Failure or revision events as identified in the registry for this cohort are summarised below. Individual data can be accessed in [Attachment 1](https://docs.google.com/spreadsheets/d/1EVtJwG2d4UhR4-VGbA0-ZSwdgSgNMbth5kMTsKR_mwI/edit#gid=0).

Table 3: Summary of PRULO Report 10 (Rotator Cuff) Procedure Survival
...
flextable::set_table_properties(TableRep103,
                                layout = "autofit",
                                width = 0.15,
                                align = "center",
                                opts_word = list(
...
```{epoxy Failure Report Rep10B Proc1}

{ifelse(nrow(FailuresRep10) == 0,
        epoxy::epoxy("There are no failures or revisions to report."),
       googlesheets4::range_write(
         ss = "https://docs.google.com/spreadsheets/d/1EVtJwG2d4UhR4-VGbA0-ZSwdgSgNMbth5kMTsKR_mwI/edit",
         data = FailuresRep10 %>% arrange(TreatmentUID),
         sheet = "Failures10",
         range = "A1:P",
         col_names = TRUE
         )
...
Table 4: Summary of PRULO Report 10 (All) Cases - Adverse Events
...
flextable::set_table_properties(TableRep104,
                                layout = "autofit",
                                #width = 0.35,
                                align = "center",
                                opts_word = list(
...
```{r Reoperations Report Rep10A Proc1}
...
SelectRep10Reop <- ComplicTable3 %>% filter(
  TreatmentUID %in% SelectRep10$TreatmentUID,
  `Reoperation Procedure` == "Yes"
  ) %>% group_by(TreatmentUID) %>% slice_min(
    ReopDelay_Surgery,
    n = 1
...
```{epoxy Reoperations Report Rep10B Proc1}



ifelse(nrow(SelectRep10Reop) == 0,
        epoxy::epoxy("There are no reoperations to report."),
       googlesheets4::range_write(
         ss = "https://docs.google.com/spreadsheets/d/1EVtJwG2d4UhR4-VGbA0-ZSwdgSgNMbth5kMTsKR_mwI/edit",
...
```{r PROMs - complete case analysis Rep10 Proc1}
# Plot what we have
...
Figure 1: Complete case analysis of QDASH(Top), WOSI Index Normalised (Bottom).

Table 5: Summary of PRULO Report 10 (All) Cases - QuickDASH and WOSI
...
flextable::set_table_properties(TableRep105,
                                layout = "autofit",
                                #width = 0.35,
                                align = "center",
                                opts_word = list(
...
Table 1: Summary of PRULO Report 11 (Rotator Cuff) Case - Patient Characteristics
...
flextable::set_table_properties(TableRep11RC1,
                                layout = "autofit",
                                width = 0.15,
                                align = "center",
                                opts_word = list(
...
Table 2: Summary of PRULO Report 11 (Rotator Cuff) Cases Pathology and Surgical Details

```{r}

flextable::set_table_properties(TableRep11RC2,
                                layout = "autofit",
                                width = 0.15,
                                align = "center",
                                opts_word = list(
...
Table 3: Summary of PRULO Report 11 (Rotator Cuff) Procedure Survival
...
flextable::set_table_properties(TableRep11RC3,
                                layout = "autofit",
                                width = 0.15,
                                align = "center",
                                opts_word = list(
...
```{epoxy Failure Report Rep11B Proc1}

{ifelse(nrow(FailuresRep11RC) == 0,
        epoxy::epoxy("There are no failures or revisions to report."),
       googlesheets4::range_write(
         ss = "https://docs.google.com/spreadsheets/d/1EVtJwG2d4UhR4-VGbA0-ZSwdgSgNMbth5kMTsKR_mwI/edit",
         data = FailuresRep11RC %>% arrange(TreatmentUID),
         sheet = "Failures11",
         range = "A1:P",
         col_names = TRUE
         )
...
Table 4: Summary of PRULO Report 11 (Rotator Cuff) Cases - Adverse Events
...
flextable::set_table_properties(TableRep11RC4,
                                layout = "autofit",
                                #width = 0.35,
                                align = "center",
                                opts_word = list(
...
```{r Reoperations Report Rep11A Proc1}
...
SelectRep11RCReop <- ComplicTable3 %>% filter(
  TreatmentUID %in% SelectRep11RC$TreatmentUID,
  `Reoperation Procedure` == "Yes"
  ) %>% group_by(TreatmentUID) %>% slice_min(
    ReopDelay_Surgery,
    n = 1
...
```{epoxy Reoperations Report Rep11B Proc1}



ifelse(nrow(SelectRep11RCReop) == 0,
        epoxy::epoxy("There are no reoperations to report."),
       googlesheets4::range_write(
         ss = "https://docs.google.com/spreadsheets/d/1EVtJwG2d4UhR4-VGbA0-ZSwdgSgNMbth5kMTsKR_mwI/edit",
...
```{r PROMs - complete case analysis Rep11 Proc1}
# Plot what we have
...
Table 5: Summary of PRULO Report 11 (Rotator Cuff) Cases - QuickDASH
...
flextable::set_table_properties(TableRep11RC5,
                                layout = "autofit",
                                #width = 0.35,
                                align = "center",
                                opts_word = list(
...
Table 6: Summary of PRULO Report 11 (Rotator Cuff) Cases - Western Ontario Rotator Cuff Index
...
flextable::set_table_properties(TableRep11RC6,
                                layout = "autofit",
                                #width = 0.35,
                                align = "center",
                                opts_word = list(
...
```{epoxy Overview Rep12}

{
  if(nrow(SelectRep12) == 1) {
    epoxy::epoxy("There is {nrow(SelectRep12)} case involving the anchor of interest. There is insufficient sample size to provide a report.")
  } else {
...
Table D: Summary of product overlap for each report sample (row), the proportion of cases within that sample that also include products included in the remaining reports (columns)
...
SelectRepAll <- SelectRep %>% dplyr::select(-TreatmentUID)

# Empty dataframe to store results
prop_table <- matrix(nrow = ncol(SelectRepAll), ncol = ncol(SelectRepAll), 0)
dimnames(prop_table) <- list(colnames(SelectRepAll), colnames(SelectRepAll))

# Loop through each column and calculate proportions
for (i in 1:ncol(SelectRepAll)) {
  # Filter data based on 1s in the current column
  filtered_data <- SelectRepAll %>% filter(
    SelectRepAll[i] == 1
    )  # Use !! for literal column name
  ...
  if (nrow(filtered_data) > 0) {
    ...
    prop_vec <- rep(0, ncol(SelectRepAll))
    prop_vec[i] <- 1
    prop_vec[-i] <- props
  } else {
    # Fill with NA if no 1s in the current column
    prop_vec <- rep(NA, ncol(SelectRepAll))
  }
...
# Convert matrix to data frame (optional)
prop_table <- as.data.frame(
  prop_table
  ) %>% mutate(
    CaseCount = colSums(SelectRepAll)
  ) %>% relocate(
    CaseCount, .before = everything()
  ) %>% gt::gt(
...
knitr::knit_print(prop_table)

```

```{r, eval = FALSE}
SelectCount <- t(colSums(SelectRepAll))
```
