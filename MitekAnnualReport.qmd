...
```{r}
#| label: tbl-intraop-complications-8
#| tbl-cap: "Summary of PRULO Report 8 (All) Cases - Intraoperative Events"

# Check if any complications exist in filtered data
has_complications <- SelectRep8 |> 
  filter(TreatmentID %in% IntraopComp$TreatmentID) |> 
  nrow() > 0
...
if (!has_complications) {
  ...
  # Calculate incidence and confidence intervals
  total_cases <- nrow(SelectRep8)
  complication_cases <- SelectRep8 |> 
    filter(TreatmentID %in% IntraopComp$TreatmentID) |> 
    nrow()
  ...
  # Generate and return the table
  IntraopComp |> 
    filter(TreatmentID %in% SelectRep8$TreatmentID) |> 
    rename(
      Description = "ICNature2",
      Intervention = "ICIntervention",
      PostopManagement = "ICPostopManagement"
...
```{r}
#| label: Reoperations-Report-Rep10B-Proc1-write
#| include: false
...
```{r}
#| label: Reoperations-Report-Rep10B-Proc1-display

{ifelse(nrow(SelectRep10Reop) == 0,
        "There are no reoperations to report.",
        ""  # Empty string when data exists
)}
...
```{r}
#| label: tbl-productusage2
#| tbl-cap: "Summary of cases with multiple product codes included per report"

SelectRep1 <- SelectRep |> 
  rowwise() |> 
