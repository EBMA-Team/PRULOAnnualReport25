...
ComplicInfection <- ComplicTable3 |> group_by(TreatmentID) |> filter(Infection == "Yes") |> slice_min(DateOccurFix, n = 1) |> ungroup
...
ProductUsage2 <- separate_longer_delim(
    ...
    ", ") |>
  left_join(
    dplyr::select(ProductTable,Reference,Description), 
            join_by
    ) |>  dplyr::mutate(
...
CONSORTData <- STROBEInput1 |> left_join(
  ...
  trialno = TreatmentID #List of the subject ID.
) |> mutate (
  ...
  induc = case_when( # list of subject ID entered the induction phase
    ...
    #!(trialno %in% SnapshotComb4$TreatmentID) ~ "Did not receive Product",
  ...
  fow2 = case_when( #reason for exclusion in the final analysis, should be missing if not excluded
    !(trialno %in% Eligible12m$TreatmentID) & (trialno %in% SnapshotConsort$TreatmentID) ~ "Insufficient fFU", # final follow up (12months)
    .default = NA_character_
  ),
...
SelectRepAll <- SelectRep |> dplyr::select(-TreatmentID)
